...
export class AuthService {
  ...
  constructor(
    ...
    this.cryptr = new Cryptr(conf.auth.secret);
  }

  async createAccessToken(userId: string) {
    // const accessToken = this.jwtService.sign({userId});
    const accessToken = sign({userId}, conf.auth.secret , { expiresIn: conf.auth.jwt_exp });
  ...
  private static jwtExtractor(request) {
    ...
    const cryptr = new Cryptr(conf.auth.secret);
    if (token) {
      try {
