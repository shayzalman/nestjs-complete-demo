...
describe('Todo Model Test', () => {
    ...
    beforeAll(async () => {
        await getMongo().then((connection) => {
            mongoose.connect(connection, { useNewUrlParser: true, useCreateIndex: true, useUnifiedTopology: true }, (err) => {
                if (err) {
                    console.error(err);
                    process.exit(1);
                }
    ...
    it('insert item successfully, but with a foreign and invalid field', async () => {
        const todoWithInvalidField = new todoModel({ title: "testing", status: 1, date: Date().toString() });
